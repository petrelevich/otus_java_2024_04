...
public class Neo4jPhoneRepository implements PhoneRepository {
    ...
    public void insert(Phone phone) {
        try (val session = driver.session()) {
            ...
            session.run(
                    "CREATE (n:Phone {id: $id, model: $model, color: " + "$color, serialNumber: $serialNumber})",
                    params);
        }
    }
    ...
    public Optional<Phone> findOne(String id) {
        try (val session = driver.session()) {
            Result result = session.run(
                    "MATCH (n:Phone {id: $id}) " + "RETURN \"{id: \" + n.id + \", "
                            + "model: \" + n.model + \", "
                            + "color: \" + n.color + \", "
                            + "serialNumber: \" + n.serialNumber + \"}\" as res",
                    Map.of("id", id));
            return Optional.ofNullable(result.single().get("res")).map(r -> mapper.fromJson(r.asString(), Phone.class));
        }
    }
    ...
    public List<Phone> findAll() {
        try (val session = driver.session()) {
            Result result = session.run("MATCH (n:Phone) " + "RETURN \"{id: \" + n.id + \", "
                    + "model: \" + n.model + \", "
                    + "color: \" + n.color + \", "
                    + "serialNumber: \" + n.serialNumber + \"}\" as res");
    ...
    public List<Phone> findAllByUserId(String userId) {
        try (val session = driver.session()) {
            Result result = session.run(
                    "MATCH (p:Phone)<--(u:PhoneUser {id: $userId}) " + "RETURN \"{id: \" + p.id + \", "
                            + "model: \" + p.model + \", "
                            + "color: \" + p.color + \", "
                            + "serialNumber: \" + p.serialNumber + \"}\" as res",
